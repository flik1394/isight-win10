...
音频包和视频包挤在同一个 DMA 缓冲里，驱动并不替我们分拣。可能出现的情况：

- **画面照旧**：驱动按包把视频 payload 拼接，音频数据落在一帧的末尾空隙里；
- **画面有条纹/错位**：音频 payload 混进了视频字节里，那就得自己分包重组
  （v17 的备选路线：不用 CMU 的 `AcquireImageEx`，改成直接 `t1394IsochSetupStream`
  自己收包，视频和音频都自己解析——`diags/audio.cpp` 的 `DoReceive()` 已经验证过
  这套参数：8 × 460800 缓冲、960 字节/包预算）。
...
- **v16** 在视频采集路径里解出音频 + 落盘取证（本次）
- **v17** audio pin：filter 直接把 PCM 交给 DirectShow
